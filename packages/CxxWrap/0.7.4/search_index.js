...
{
    "location": "#Linking-with-the-C-library-1",
    "page": "Readme",
    "title": "Linking with the C++ library",
    "category": "section",
    "text": "The library (in deps/src/jlcxx) is built using CMake, so it can be found from another CMake project using the following line in a CMakeLists.txt:find_package(CxxWrap)The CMake variable CxxWrap_DIR should be set to the directory containing the CxxWrapConfig.cmake, typically ~/.julia/<Julia version>/CxxWrap/deps/usr/lib/cmake. One can then link using:target_link_libraries(your_own_lib CxxWrap::jlcxx)A complete CMakeLists.txt is at deps/src/examples/CMakeLists.txt."
},
