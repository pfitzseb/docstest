...

{
    ...
    "title": "Installation",
    "category": "section",
    "text": "julia>]\npkg> add HDF5If you run into installation errors, try to install the HDF5 library manually (version 1.8+ is required):Debian/(K)Ubuntu: apt-get -u install hdf5-tools\nOSX: brew install hdf5 (using Homebrew, please verify homebrew version is 1.8+ using brew --version)\nWindows: Not recommended, as other HDF5 binaries may be compiled against a different C runtime from the Julia binary, which can cause Julia to crash when freeing memory allocated by libhdf5.If you\'ve installed the library but discover that Julia is not finding it, you can add the path to Julia\'s Libdl.DL_LOAD_PATH variable, e.g.,push!(Libdl.DL_LOAD_PATH, \"/opt/local/lib\")\nPkg.build(\"HDF5\")Inserting this command into your .juliarc.jl file will cause this to happen automatically each time you start Julia.If you\'re on Linux but you do not have root privileges on your machine (and you can\'t persuade the sysadmin to install the libraries for you), you can download the binaries and place them somewhere in your home directory. To use HDF5, you\'ll have to start julia asLD_LIBRARY_PATH=/path/to/hdf5/libs juliaYou can set up an alias so this happens for you automatically each time you start julia."
},
