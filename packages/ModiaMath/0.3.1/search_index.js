...

{
    ...
    "title": "Version 0.2.4",
    "category": "section",
    "text": "Non-backwards compatible change of ModiaMath.plot(..): The function was changed to only support result dictionaries of the type Dict{AbstractString,Any} to simplify implementation and maintenance and to use the identical dictionary type as in Modia.\nModiaMath.plot(..) improved:\nFor the dictionary type used by Modia, all elements of a variable vector are plotted by giving the variable name, or one specific element of the variable vector is plotted by giving the name and the vector index.\nA new keyword argument maxLegend=10 introduced: If the number of entries in a legend exceeds this number, no legend is included in the plot. Note, the curves can still be identified by clicking in the tool bar of the plot window on button Edit axis, curve ...\nIf the plot function is used in the wrong way (e.g. signals shall be plotted that are not in the result dictionary, or selecting a variable vector as x-axis), warning messages are printed and the call is ignored.\nDependent packages updated to their newest versions. Especially, warnings from Unitful do no longer occur, due to the update to version 0.12.0.\nIssue with PyPlot warnings fixed: When ModiaMath was used in another package (e.g. in Modia or Modia3D), strange warning messages appeared when importing it. This issue was fixed with the solution sketched in discourse."
},
...

{
    ...
    "title": "Version 0.2.1",
    "category": "section",
    "text": "Adapted to Julia 0.7 and 1.0 (including using new package manager via Project.toml, Manifest.toml files and adapting the README.md and documentation files).\nButtons for Travis CL, code coverage and docs added to README.md file. (this includes adaptations to .travis.yml file). Note, Appveyor CL is not yet activated (although appveyor.yml file is present).\nInterface call to KINSOL slightly improved to avoid a gc-crash.\nIn case KINSOL fails, more run-time information is added to the error message.\nIndentation was changed consistently to 4 spaces."
},
