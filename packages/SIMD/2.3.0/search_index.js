...

{
    ...
    "title": "Overview",
    "category": "section",
    "text": "This package allows programmers to explicitly SIMD-vectorize their Julia code. Ideally, the compiler (Julia and LLVM) would be able to do this automatically, especially for straightforwardly written code. In practice, this does not always work (for a variety of reasons), and the programmer is often left with uncertainty as to whether the code was actually vectorized. It is usually necessary to look at the generated machine code to determine whether the compiler actually vectorized the code.By exposing SIMD vector types and corresponding operations, the programmer can explicitly vectorize their code. While this does not guarantee that the generated machine code is efficient, it relieves the compiler from determining whether it is legal to vectorize the code, deciding whether it is beneficial to do so, and rearranging the code to synthesize vector instructions.Here is a simple example for a manually vectorized code that adds two arrays:using SIMD\nfunction vadd!(xs::Vector{T}, ys::Vector{T}, ::Type{Vec{N,T}}) where {N, T}\n    @assert length(ys) == length(xs)\n    @assert length(xs) % N == 0\n    lane = VecRange{N}(0)\n    @inbounds for i in 1:N:length(xs)\n        xs[lane + i] += ys[lane + i]\n    end\nendTo simplify this example code, the vector type that should be used (Vec{N,T}) is passed in explicitly as additional type argument. This routine is e.g. called as vadd!(xs, ys, Vec{8,Float64})."
},
...

{
    ...
    "category": "section",
    "text": "When using explicit SIMD vectorization, it is convenient to allocate arrays still as arrays of scalars, not as arrays of vectors. The vload and vstore functions allow reading vectors from and writing vectors into arrays, accessing several contiguous array elements.arr = Vector{Float64}(undef, 100)\n...\nxs = vload(Vec{4,Float64}, arr, i)\n...\nvstore(xs, arr, i)The vload call reads a vector of size 4 from the array, i.e. it reads arr[i:i+3]. Similarly, the vstore call writes the vector xs to the four array elements arr[i:i+3].When the values to be read are stored in non-contiguous locations, the vgather function can be used to load them into a vector (so-called gather operation).idx = Vec((1, 5, 6, 9))\nvgather(arr, idx)Likewise, storing to non-contiguous locations (scatter) can be done by the vscatter function.arr = zeros(10)\nv = Vec((1.0, 2.0, 3.0, 4.0))\nidx = Vec((1, 3, 4, 7))\nvscatter(v, arr, idx)Above vload, vstore, vgather and vscatter can be written using the indexing notation:i = 1\nlane = VecRange{4}(0)\nv = arr[lane + i]             # vload\narr[lane + i] = v             # vstore\nidx = Vec((1, 3, 4, 7))\nv = arr[idx]                  # vgather\narr[idx] = v                  # vscatter"
},
...
{
    "location": "autodocs/#SIMD.VecRange",
    "page": "Docstrings",
    "title": "SIMD.VecRange",
    "category": "type",
...

{
    ...
    "title": "Docstrings",
    "category": "page",
    "text": "SIMD.BoolTypesSIMD.FloatingTypesSIMD.IndexTypesSIMD.IntTypesSIMD.IntegerTypesSIMD.SIMDSIMD.ScalarTypesSIMD.UIntTypesSIMD.VESIMD.VecSIMD.VecRangeSIMD._checkaritySIMD._extractmaskSIMD._maskedidxSIMD._preprocessindicesSIMD.array2vectorSIMD.evalSIMD.exponent_bitsSIMD.exponent_maskSIMD.extendvectorSIMD.getneutralSIMD.includeSIMD.int_typeSIMD.llvmconstSIMD.llvminsSIMD.llvmtypeSIMD.llvmtypedconstSIMD.llvmwrapSIMD.llvmwrapreduceSIMD.llvmwrapshiftSIMD.nextpow2SIMD.scalar2vectorSIMD.setindexSIMD.shufflevectorSIMD.shufflevector_instrsSIMD.sign_bitsSIMD.sign_maskSIMD.significand_bitsSIMD.significand_maskSIMD.subvectorSIMD.suffixSIMD.uint_typeSIMD.vallocSIMD.vector2arraySIMD.vgatherSIMD.vgatheraSIMD.vifelseSIMD.vloadSIMD.vloadaSIMD.vscatterSIMD.vscatteraSIMD.vstoreSIMD.vstorea"
},
